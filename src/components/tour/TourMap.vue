<template>
  <div class="map-container">
    ...
    <div class="tour-plan-sidebar" :class="{active: isPlanShow}">
      ...
      <div class="close-button" @click="onClickSidebarCloseBtn(plan)"><span class="material-symbols-outlined">
chevron_right
</span></div>
      <div class="plans">
        <draggable v-model="planList" v-bind="dragOptions"
        @start="drag = true"
        @end="drag = false">
          <transition-group>
            <div
            ...
            :key="index"
          >
          <span class="material-symbols-outlined" @click="onClickPlanClose(plan)">
            close
          </span>
      ...
      <div class="button-area">
        <div class="reset-button" @click="onClickResetPlanBtn()"><span class="material-symbols-outlined">
delete
</span></div>
        <div class="resgister-button"><span class="material-symbols-outlined">
arrow_forward
</span></div>
      </div>
    </div>
  ...
  methods: {
    ...
    onClickPlanAddBtn(place) {
      for (let i = 0; i < this.planList.length; i++) {
        if ((this.planList[i].contentid + this.planList[i].contenttypeid) === (place.contentid + place.contenttypeid)) {
          return;
        }
      }
    ...
    onClickPlanClose(place) {
      console.log(place);
      this.planList = this.planList.filter((plan) => (plan.contentid + plan.contenttypeid) !== (place.contentid + place.contenttypeid))
    },
    onClickResetPlanBtn() {
      this.planList = [];
    },
...
return content;
    ...
    if (this.totalDistanceOverlay) { // 커스텀오버레이가 생성된 상태이면
      console.log('이미있음!!!');
